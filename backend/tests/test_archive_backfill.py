"""The archive must keep fetching history, and the cursor must not lie about it.

Two halves, landed separately.

`jobs/archive_scorer.py` (main, 2 Sep) turns raw_documents into scored
entity_mentions. `jobs/archive_backfill.py` (here) fetches the documents for it
to score — without it, every backfill source is a manual CLI and the scorer
drains a backlog that stops being refilled.

The wayback cursor fix these tests exercise also landed on main on 2 Sep,
independently and equivalently: it never called read_cursor, so every run
restarted from --since; it wrote the cursor as `until` — the END of the
requested range — however far it actually got; and write_cursor sat outside the
except, so a host that raised still recorded "completed through until".

These tests were written against a separate fix and now pass unchanged against
main's, which is the useful outcome: they are a behavioural guard on whichever
implementation is in the tree, not a description of one.
"""
