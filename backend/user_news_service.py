...
# the story — the same paragraph on every card. Any candidate summary matching
# these is discarded rather than shown.
...
def is_usable_summary(text: str, title: str = "") -> bool:
    ...
    and text that merely restates the headline (which is what the feed has
    been emitting for every article).
    """
    ...
    if title:
        ...
        if norm(cleaned) == norm(title) or norm(cleaned).startswith(norm(title)) and len(cleaned) < len(title) * 1.3:
            return False
    return True
...
class UserNewsService:
    ...
    def __init__(self):
        ...
        # what now populates the card summary without any scraping.
        #
        #   oilprice   200  15 entries  10 with_summary   <- best source
        #   eia_today  200  16 entries  10 with_summary   <- added; the old
        #                   .../weekly/includes/newsletter_rss.xml 404s
        #   investing  200  10 entries   0 with_summary   <- headlines only,
        #                   and the site blocks scraping, so its cards carry
        #                   the title until another source covers the story
        #   reuters via Google News returns 503 from datacenter IPs (Railway)
        #                   even though it works from residential ones — kept
        #                   because it degrades to zero rather than erroring.
        self.rss_feeds = {
    ...
    # only investing.com. Fetching the bytes ourselves with real headers and
    # handing them to feedparser fixes it. Verified: data_sources.py uses the
    # same UA and pulls 15 OilPrice articles per tick from the same host.
    _FEED_HEADERS = {
        'User-Agent': (
            ...
            try:
                ...
                for entry in feed.entries[:10]:  # Limit entries per feed
                    ...
                    if relevant or not commodities:  # Include if relevant or no specific commodities
                        ...
                        # reliable summary we have: no scraping, no bot walls,
                        ...
                        # request, so sleeping on them was costing ~0.5s each
                        # (20s+ across four feeds) for nothing.
                        if self.summarizer and url and not rss_summary:
