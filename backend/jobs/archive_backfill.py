"""Scheduled archive backfill — walks history backwards, a little at a time.

Why this exists
---------------
The archive only ever grew forward. `news_fetcher` writes what the RSS feeds
are showing right now, every ten minutes, and nothing ever reached backwards:
every backfill source was a manual CLI, nothing scheduled them, and
`backfill_cursors` held no rows at all.

The orchestrator was already designed for progressive scheduling -- `run_all
--max-days N` bounds each source and the cursor makes a bounded run a
checkpoint rather than a partial failure. What was missing is a caller.
...
log a warning and return 0 -- harmless, but pointless to schedule. Of the
remaining four, only `gdelt` and (after this change) `wayback` both resume from
a cursor and bound their work; `cftc_cot` resumes by calendar year, and
...
`ARCHIVE_BACKFILL_SOURCES` makes that list explicit rather than implicit.
...
history over weeks of wall-clock at negligible sustained load -- which is the
right trade while the app is launching and nobody is waiting on the archive.
