        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    {/* Opens on the RIGHT, under the avatar that opened it.
                        It used to fly in from the left while its trigger sat in
                        the top-right corner, so every interaction was: click
                        right, read left, close left, go back right. A panel
                        belongs on the side of the control that summons it —
                        which is also what Claude does, with the account button
                        and its menu both at the bottom-left. */}
                    <motion.div
                        initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                        transition={{ type: 'spring', damping: 25, stiffness: 200 }}
                        className="fixed top-0 right-0 h-full w-full max-w-md bg-[#121212] border-l border-[#333] z-50 overflow-y-auto"
                    >
                        {/* Close is on the right too, nearest the avatar and the
                            screen edge the panel came from. */}
                        ...
                        <div className="p-4">
                            ...
                            <div className="bg-[#1E1E1E] border border-[#333] rounded-xl overflow-hidden">
                                ...
                                <a
                                    href="https://dashboard.integramarkets.app/docs"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center justify-between px-4 py-3 border-b border-[#333] hover:bg-white/5"
                                >
