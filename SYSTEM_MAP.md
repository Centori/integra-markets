...
| OTA never arrives | THREE separate causes, check all: (1) `updates.url` ≠ `projectId`; (2) runtimeVersion mismatch (OTA runtime must equal the installed build's — currently 1.0.3); (3) **the CHANNEL has no BRANCH pointed at it** — `eas update --branch X` publishes to a branch, but builds poll a CHANNEL; a same-named channel does NOT auto-map. Must run `eas channel:edit <channel> --branch <branch>` once. | app.json `updates.url` == `extra.eas.projectId`; `eas channel:view production` must show a branch + update |
| OTA published but device won't boot after | Local export corrupted (historical "AppRegistry n=0") | Verify BEFORE publishing: `strings` the local .hbc for `AppRegistry`/`registerComponent`/`runApplication` and compare counts against a known-good embedded bundle from an IPA; if any are 0 where embedded has them, DO NOT publish. Recover with `eas update:roll-back-to-embedded --branch production --runtime-version 1.0.3` |
...
5. ~~Verify sumy/newspaper3k load in the container~~ — **RESOLVED.** Root cause was NOT the deps (live log confirms `NLTK: True, Sumy: True, Newspaper: True` — the real build uses full requirements.txt; the committed Dockerfile's `requirements-light.txt` is stale/misleading). The actual cause: `backend/article_summarizer.py` was deleted on the main lineage (b38b61fa) while the import stayed → ImportError → permanent fallback. Restored from build 64; `/api/summarize/article` returns real summaries again.
6. **Learning loop (UCB1 bandit + sentiment MLP) is built but DORMANT — zero data flowing in.** `services/learning_loop.py` is a real implementation (torch MLP, ReplayBuffer, UCB1 KeywordBandit), but live `/api/metrics/learning` shows `replay_size:0, n_keywords_tracked:0, bandit_total_evaluations:0`. Three disconnects: **(a)** all 4 learning tables (`prediction_outcomes`, `predictions`, `learning_events`, `keyword_stats`) return 404 — migration `supabase/migrations/20260527_learning_loop.sql` was never applied; **(b)** mobile poll votes write straight to `sentiment_votes` via `supabaseService.submitPollVote`, bypassing `POST /api/feedback` (the only path that calls `loop.capture_experience`) — so votes never become reward signal; **(c)** user personal inputs (alert keywords, web sources, tracked commodities) feed `user_news_service` for FEED PERSONALIZATION only — by design they were never wired as learning features (the bandit learns article-extracted tokens → 24h outcome, not user-preference → outcome). To activate: apply the migration, then route poll votes (and/or article-outcome jobs) through `/api/feedback`.
